...
private:
  ...
  void geojson_waypt_pr(const GBWaypoint* waypoint) const;
  static GBWaypoint* waypoint_from_coordinates(const QJsonArray& coordinates);
  static void routes_from_polygon_coordinates(const QJsonArray& polygon);
  void geojson_track_hdr(const route_head* track);
  void geojson_track_disp(const GBWaypoint* trackpoint) const;
