...
private:
  ...
  static QByteArray latlon2str(const GBWaypoint* wpt);
  static QByteArray date2str(const gpsbabel::DateTime& dt);
  static QByteArray tod2str(const gpsbabel::DateTime& tod);
  void wr_header();
  void wr_task_wpt_name(const GBWaypoint* wpt, const char* alt_name);
  void wr_task_hdr(const route_head* rte, unsigned int task_num);
  void wr_task_wpt(const GBWaypoint* wpt);
  void wr_task_tlr(const route_head* rte);
  void wr_tasks();
  void wr_fix_record(const GBWaypoint* wpt, int pres_alt, int gnss_alt);
...
struct igc_fsdata : public FormatSpecificData {
  ...
  bool set_value(IgcFormat::igc_ext_type_t type, double value, GBWaypoint *wp = nullptr)
  {
    bool success = true;
    switch (type) {
