...
private:
  ...
  public:
    ...
    double total {0};
    int count {0};
    const GBWaypoint* prev_wpt {nullptr};
    const GBWaypoint* first_wpt {nullptr};
    const GBWaypoint* last_wpt {nullptr};
  };
  ...
  void convert_datum(const GBWaypoint* wpt, double* dest_lat, double* dest_lon) const;
  void enum_waypt_cb(const GBWaypoint* wpt);
  void prework_hdr_cb(const route_head* unused);
  void prework_tlr_cb(const route_head* unused);
  void prework_wpt_cb(const GBWaypoint* wpt);
  void print_position(const GBWaypoint* wpt);
  void print_date_and_time(time_t time, bool time_only);
  void print_categories(uint16_t categories);
  void print_course(const GBWaypoint* A, const GBWaypoint* B);
