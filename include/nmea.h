...
private:
  ...
  GBWaypoint* nmea_new_wpt();
  void nmea_add_wpt(GBWaypoint* wpt, route_head* trk) const;
  static void nmea_release_wpt(GBWaypoint* wpt);
  void nmea_set_waypoint_time(GBWaypoint* wpt, QDateTime* prev, const QDate& date, const QTime& time);
  ...
  int hunt_sirf();
  void nmea_wayptpr(const GBWaypoint* wpt) const;
  void nmea_track_init(const route_head* unused);
  void nmea_trackpt_pr(const GBWaypoint* wpt);
  ...
  GBWaypoint* curr_waypt{};
  GBWaypoint* last_waypt{};
  void* gbser_handle{};
  QString posn_fname;
  QList<GBWaypoint*> pcmpt_head;
