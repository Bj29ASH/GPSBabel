...
private:
  ...
  struct FitCourseRecordPoint {
    FitCourseRecordPoint(const GBWaypoint& wpt, bool is_course_point, unsigned int course_point_type = kCoursePointTypeGeneric)
      : lat(wpt.latitude),
        lon(wpt.longitude),
...
// misc. constants for message fields
  ...
  void fit_collect_trackpt(const GBWaypoint* waypointp);
  void fit_collect_track_tlr(const route_head* rte);
  void fit_collect_waypt(const GBWaypoint* waypointp);
