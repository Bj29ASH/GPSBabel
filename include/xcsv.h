...
private:
  ...
  static garmin_fs_t* gmsd_init(GBWaypoint* wpt);
  static void xcsv_parse_val(const QString& value, GBWaypoint* wpt, const XcsvStyle::field_map& fmp, xcsv_parse_data* parse_data, int line_no);
  void xcsv_resetpathlen(const route_head* head);
  void xcsv_waypt_pr(const GBWaypoint* wpt);
