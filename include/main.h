...
public:
  ...
  void waypt_disp(const GBWaypoint* wpt)
  {
...
//    if (qargs.at(argn).at(0).toLatin1() != '-') {
  ...
  if (!ovecs) {
    auto waypt_disp_lambda = [&fbOutput](const GBWaypoint* wpt)->void {
  ...
  if (global_opts.masked_objective & POSNDATAMASK) {
    ...
    while (!tracking_status.request_terminate) {
      GBWaypoint* wpt = ivecs->rd_position(&tracking_status);
...
static int
babel_export_fun(const QString &inFormat, const QString &inFilePath, const QString &outFormat, const QString
&outFilePath, char type)
{
	...
	ivecs = Vecs::Instance().find_vec(inFormat);
	if ( !ivecs )
	{
		fatal("Input type '%s' not recognized\n", qPrintable(inFormat));
	}

	printf("Input Format Recognized :: %s", ivecs.fmtname.toStdString().c_str());

	ovecs = Vecs::Instance().find_vec(outFormat);
	if ( !ovecs )
	{
		fatal("Output type '%s' not recognized\n", qPrintable(outFormat));
	}

	printf("\nOutput Format Recognized :: %s", ovecs.fmtname.toStdString().c_str());
	...
	printf("\nInput File Name Recognized :: %s", fname.toStdString().c_str());
	...
	printf("Route Data Start\n");
	printf("InterData :: RouteCount :: %d\n", route_count());

	auto rteList = get_global_route_list();

	for (int i = 0; i < rteList.count(); ++i)
	{
		printf("InterData :: Route Name :: %s\n", rteList[i]->rte_name.toStdString().c_str());
		printf("InterData :: Route WptCount :: %d\n",rteList[i]->rte_waypt_ct());

		for (int j = 0; j < rteList[i]->waypoint_list.count(); ++j)
		{
			printf("InterData :: Route :: Wpt :: Name :: %s\n", rteList[i]->waypoint_list[j]->shortname.toStdString().c_str());
			printf("InterData :: Route :: Wpt :: Alt :: %f\n", rteList[i]->waypoint_list[j]->altitude);
			printf("InterData :: Route :: Wpt :: Date :: %s\n", rteList[i]->waypoint_list[j]->creation_time.date().toString("dd/MM/yyyy").toStdString().c_str());
			printf("InterData :: Route :: Wpt :: Time :: %s\n", rteList[i]->waypoint_list[j]->creation_time.time().addSecs(19800).toString("hh:mm:ss").toStdString().c_str());
		}

	}

	printf("Route Data End\n");
	...
	printf("\nOutput File Name Recognized :: %s", ofname.toStdString().c_str());
	...
	printf("\nCreated \' %s \' file successfully at \" %s \"", ovecs.fmtname.toStdString().c_str(), ofname.toStdString().c_str());

	return 0;
}

static int
babel_multiple_export_fun(const QString &inFormat, const QList<QString> &inFilePaths, const QString &outFormat, const QString &outFilePath, char type)
{
	//Vecs::Instance().init_vecs();
	//FilterVecs::Instance().init_filter_vecs();
	session_init();
...
static int
babel_read_routes(const QString &inFormat, const QString &inFilePath, RouteList &routeList, char type)
{
	//Vecs::Instance().init_vecs();
	//FilterVecs::Instance().init_filter_vecs();
	session_init();
	...
	routeList = get_global_route_list();

//	route_deinit();
	waypt_deinit();
	session_exit();

	printf("\nCopied to route list successfull. Route Count \' %d \'", routeList.count());

	return 0;
}

int
main_fun(int argc, char* argv[])
{
#ifdef AFL_INPUT_FUZZING
  AFL_INIT_ARGV();
#endif
