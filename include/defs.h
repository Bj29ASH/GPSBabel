...
public:
  ...
  GBWaypoint();
  ~GBWaypoint();
  GBWaypoint(const GBWaypoint& other);
  GBWaypoint& operator=(const GBWaypoint& rhs);
...
#define GEN_WAYPT_METHODS(field) \
  ...
  bool field##s_equal(const GBWaypoint& other) const \
  { \
...
using waypt_cb = void (*)(const GBWaypoint*);

// TODO: Consider using composition instead of private inheritance.
class WaypointList : public QList<GBWaypoint*>
{
public:
  void waypt_add(GBWaypoint* wpt); // a.k.a. append(), push_back()
  void add_rte_waypt(int waypt_ct, GBWaypoint* wpt, bool synth, QStringView namepart, int number_digits);
  ...
  void waypt_del(GBWaypoint* wpt); // a.k.a. erase()
  void del_marked_wpts();
  // FIXME: Generally it is inefficient to use an element pointer or reference to define the element to be deleted, use iterator instead,
  //        and/or implement pop_back() a.k.a. removeLast(), and/or pop_front() a.k.a. removeFirst().
  void del_rte_waypt(GBWaypoint* wpt);
  void waypt_compute_bounds(bounds* bounds) const;
  GBWaypoint* find_waypt_by_name(const QString& name) const;
  ...
  // public types
  using QList<GBWaypoint*>::const_iterator;
  using QList<GBWaypoint*>::const_reverse_iterator;
  using QList<GBWaypoint*>::iterator;
  using QList<GBWaypoint*>::reverse_iterator;
  // public functions
  using QList<GBWaypoint*>::back; // a.k.a. last()
  using QList<GBWaypoint*>::begin;
  using QList<GBWaypoint*>::cbegin;
  using QList<GBWaypoint*>::cend;
  using QList<GBWaypoint*>::count; // a.k.a. size()
  using QList<GBWaypoint*>::crbegin;
  using QList<GBWaypoint*>::crend;
  using QList<GBWaypoint*>::detach; // silence Qt6 foreach warnings
  using QList<GBWaypoint*>::empty; // a.k.a. isEmpty()
  using QList<GBWaypoint*>::end;
  using QList<GBWaypoint*>::front; // a.k.a. first()
  using QList<GBWaypoint*>::rbegin;
  using QList<GBWaypoint*>::rend;
  using QList<GBWaypoint*>::size_type;
};

void waypt_init();
//void update_common_traits(const Waypoint* wpt);
void waypt_add(GBWaypoint* wpt);
void waypt_del(GBWaypoint* wpt);
void del_marked_wpts();
...
void waypt_add_to_bounds(bounds* bounds, const GBWaypoint* waypointp);
void waypt_compute_bounds(bounds* bounds);
GBWaypoint* find_waypt_by_name(const QString& name);
...
void waypt_add_url(GBWaypoint* wpt, const QString& link,
                   const QString& url_link_text);
void waypt_add_url(GBWaypoint* wpt, const QString& link,
                   const QString& url_link_text,
                   const QString& url_link_type);
double gcgeodist(double lat1, double lon1, double lat2, double lon2);
double waypt_time(const GBWaypoint* wpt);
double waypt_distance_ex(const GBWaypoint* A, const GBWaypoint* B);
double waypt_distance(const GBWaypoint* A, const GBWaypoint* B);
double waypt_speed_ex(const GBWaypoint* A, const GBWaypoint* B);
double waypt_speed(const GBWaypoint* A, const GBWaypoint* B);
double waypt_vertical_speed(const GBWaypoint* A, const GBWaypoint* B);
double waypt_gradient(const GBWaypoint* A, const GBWaypoint* B);
double waypt_course(const GBWaypoint* A, const GBWaypoint* B);
...
{
  int i = 0;
  foreach (GBWaypoint* waypointp, *this) {
...
// TODO: Consider using composition instead of private inheritance.
class RouteList : public QList<route_head*>
{
public:
  ...
  void add_wpt(route_head* rte, GBWaypoint* wpt, bool synth, QStringView namepart, int number_digits);
  // FIXME: Generally it is inefficient to use an element pointer or reference to define the insertion point, use iterator instead.
  void del_wpt(route_head* rte, GBWaypoint* wpt);
...
void route_init();
const RouteList get_global_route_list();
...
void route_add_wpt(route_head* rte, GBWaypoint* wpt, QStringView namepart = u"RPT", int number_digits = 3);
void track_add_wpt(route_head* rte, GBWaypoint* wpt, QStringView namepart = u"RPT", int number_digits = 3);
void route_del_wpt(route_head* rte, GBWaypoint* wpt);
void track_del_wpt(route_head* rte, GBWaypoint* wpt);
...
// cb != nullptr, caught with an overload of route_disp
  foreach (const GBWaypoint* waypointp, rh->waypoint_list) {
    cb(waypointp);
  }
}
