...
public:
  ...
  GBWaypoint* rd_position(posn_status* status) override;
...
private:
  ...
  static unsigned int checkWayPointIsAtSplit(GBWaypoint* wpt, GPS_SLap** laps, int nlaps);
  void track_read();
  void route_read();
  static void pvt2wpt(GPS_PPvt_Data pvt, GBWaypoint* wpt);
  static GPS_SWay* sane_GPS_Way_New();
  static int waypt_write_cb(GPS_SWay** /* unused */);
  static const char* get_gc_info(const GBWaypoint* wpt);
  int waypoint_prepare();
  void waypoint_write();
  void route_hdr_pr(const route_head* rte);
  void route_waypt_pr(const GBWaypoint* wpt);
  void route_write();
  void track_hdr_pr(const route_head* trk_head);
  void track_waypt_pr(const GBWaypoint* wpt);
  int track_prepare();
  void track_write();
  void course_write();
  static const char* d103_symbol_from_icon_number(unsigned int n);
  static int d103_icon_number_from_symbol(const QString& s);
  void garmin_fs_garmin_after_read(GPS_PWay way, GBWaypoint* wpt, int protoid);
  void garmin_fs_garmin_before_write(const GBWaypoint* wpt, GPS_PWay way, int protoid);
